...
def func_get_player_stats(df_runs):
    ...
    stats_runs['Runs Back']  = (stats_runs['end_x'] <= stats_runs['start_x']).astype(int)
       
    
    df_runs_selected = stats_runs[["team_name","player","period","Distance",'Runs into box','max_speed','Runs Back','Forward runs']]
...
def show_runs():
    ...
    if uploaded_track_file is None:
        ...
        tracks_columns = ["Team", "Player",'Jersey','Period', "Frame Start",'Frame End']
        
        df_runs = df_runs[tracks_columns]
        
        df_runs = df_runs.sort_values(by=['Team', 'Player', 'Frame Start'])
...
if __name__ == "__main__":
    ...
    if uploaded_runs_file is not None: 
        ...
        tab1, tab2, tab3 = st.tabs(["Runs by players in final third :twisted_rightwards_arrows:","Average run direction :repeat:","Display Runs:film_projector:"])  
